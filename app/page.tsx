...
type ProteinDomain = { name:string; start:number; end:number };
...
function parseDomains(text:string):ProteinDomain[]{
  return text.split(/\r?\n/).map(line=>line.trim()).filter(Boolean).map((line,index)=>{
    const parts=line.split(/[,\t]+/).map(x=>x.trim());
    if(parts.length<3)throw new Error(`Domain line ${index+1} must use: name,start,end`);
    const start=Number(parts[parts.length-2]),end=Number(parts[parts.length-1]),name=parts.slice(0,-2).join(", ");
    if(!name||!Number.isFinite(start)||!Number.isFinite(end)||start<1||end<start)throw new Error(`Invalid domain on line ${index+1}.`);
    return {name,start:Math.floor(start),end:Math.floor(end)};
  });
}
...
function pct(position:number,length:number){if(length<=1)return 0;return Math.max(0,Math.min(100,((position-1)/(length-1))*100));}
...
export default function Home(){
  ...
  const [domainText,setDomainText]=useState(""),[domainError,setDomainError]=useState("");
  const [hits,setHits]=useState<Hit[]|null>(null),[selectedHit,setSelectedHit]=useState<Hit|null>(null),[error,setError]=useState(""),[query,setQuery]=useState("");
  ...
  const domains=useMemo(()=>{try{setDomainError("");return parseDomains(domainText)}catch(problem){setDomainError(problem instanceof Error?problem.message:"Invalid domain list.");return[]}},[domainText]);
  ...
  const targetReference=targetCandidates.find(r=>r.id===targetReferenceId)??targetCandidates[0];
  const proteinLength=targetReference?targetReference.sequence.replace(/-/g,"").length:0;
  ...
  function analyse(){
    try{
      ...
      if(domainText.trim())parseDomains(domainText);
      ...
      setHits(foundHits);setSelectedHit(foundHits[0]??null);setSummary({sequences:records.length,target:target.length,comparison:comparison.length,length:records[0].sequence.length,requested:ids.length,found:found.length,missing});
      setTargetReferenceId(targetRef.id);setReferenceId(reference.id);setError("");
    }catch(problem){setError(problem instanceof Error?problem.message:"The files could not be analysed.");setHits(null);setSelectedHit(null);}
  }
  function loadDemo(){setFasta(demoFasta);setTargetIdentifiers(demoIdentifiers);setTargetName("Hardy");setTargetReferenceId("Hardy_CAN-006");setReferenceId("Ubiq_26695");setThreshold(100);setAllowedComparisonMatches(0);setDomainText("N-terminal,1,5\nCore,6,14");setHits(null);setSelectedHit(null);setError("");}
  ...
  return <main>
    ...
    <section className="workspace"><div className="stepTitle"><span>01</span><div><h2>Add your alignment</h2><p>Upload a protein FASTA in which all sequences are already aligned.</p></div></div><div className="uploadSingle"><label className={`dropzone ${fasta?"filled":""}`}><input type="file" accept=".fa,.faa,.fasta,.fas,.aln,.txt" onChange={e=>e.target.files?.[0]&&readFile(e.target.files[0],setFasta)}/><span className="fileIcon">{fasta?"✓":">_"}</span><strong>{fasta?"Aligned FASTA loaded":"Aligned protein FASTA"}</strong><small>{fasta?`${parsed.length} sequences detected`:".faa, .fasta, .fa or .txt"}</small></label></div>
      ...
      <div className="stepTitle second"><span>04</span><div><h2>Add protein domains <small className="optional">optional</small></h2><p>Overlay known domains on the result map using the tested-group numbering strain.</p></div></div>
      <label className="domainInput">Domains — one per line<textarea value={domainText} onChange={e=>setDomainText(e.target.value)} rows={5} placeholder={"Sensor domain,29,131\nTransmembrane,132,151\nC-terminal domain,300,448"}/><small>Format: <b>name,start,end</b>. Coordinates must match the selected tested-group numbering strain.</small></label>{domainError&&<div className="domainError">{domainError}</div>}
      <div className="autoCompare"><span>AUTOMATIC COMPARISON</span><p><b>{targetName}:</b> sequences matching a listed identifier. <b>Everything else:</b> all remaining FASTA sequences.</p></div><div className="rule"><span>DETECTION RULE</span><p>The group residue must occur in <b>at least {threshold}% of tested sequences</b> and in <b>no more than {allowedComparisonMatches} comparison {allowedComparisonMatches===1?"sequence":"sequences"}</b>. Gaps and unknown residues cannot define the tested group.</p></div>{error&&<div className="error" role="alert">{error}</div>}<button className="analyse" disabled={!fasta||!targetIdentifiers.trim()||!!domainError} onClick={analyse}>Find {targetName}-{allowedComparisonMatches===0?"exclusive":"associated"} residues <span>→</span></button>
    </section>
    {hits!==null&&<section className="results"><div className="resultsHead"><div><p className="eyebrow">RESULTS · {threshold}% THRESHOLD · MAX {allowedComparisonMatches} COMPARISON {allowedComparisonMatches===1?"MATCH":"MATCHES"}</p><h2>{hits.length} {targetName}-{allowedComparisonMatches===0?"exclusive":"associated"} {hits.length===1?"position":"positions"}</h2><p>{summary.target} tested · {summary.comparison} comparison · {summary.found}/{summary.requested} identifiers found</p></div><button className="download" onClick={downloadCsv}>Download CSV ↓</button></div>
      ...
      {hits.length&&proteinLength>0?<section className="proteinMapCard"><div className="mapHead"><div><p className="eyebrow">PROTEIN MAP</p><h3>{targetReferenceId}</h3><p>{proteinLength} aa · {hits.filter(h=>h.targetPosition!==null).length} mapped result{hits.filter(h=>h.targetPosition!==null).length===1?"":"s"}{domains.length?` · ${domains.length} domain${domains.length===1?"":"s"}`:""}</p></div><div className="mapLegend"><span><i className="legendHit"/>Result</span>{domains.length>0&&<span><i className="legendDomain"/>Domain</span>}</div></div>
        <div className="proteinMap" aria-label="Protein result map">
          <div className="mapAxis"><span>1</span><span>{proteinLength}</span></div>
          <div className="domainLane">{domains.map((domain,index)=>{const start=Math.min(domain.start,proteinLength),end=Math.min(domain.end,proteinLength);if(start>proteinLength)return null;return <div key={`${domain.name}-${index}`} className={`domainBlock domainTone${index%4}`} style={{left:`${pct(start,proteinLength)}%`,width:`${Math.max(1,pct(end,proteinLength)-pct(start,proteinLength))}%`}} title={`${domain.name}: ${domain.start}–${domain.end}`}><span>{domain.name}</span></div>})}</div>
          <div className="proteinTrack"/>
          <div className="hitLane">{hits.map((hit,index)=>hit.targetPosition===null?null:<button key={hit.alignmentPosition} className={`hitMarker ${selectedHit?.alignmentPosition===hit.alignmentPosition?"selected":""}`} style={{left:`${pct(hit.targetPosition,proteinLength)}%`,top:`${8+(index%3)*12}px`}} onClick={()=>setSelectedHit(hit)} title={`Position ${hit.targetPosition}: ${hit.targetAA}; ${hit.targetPresent}/${hit.targetTotal} tested; ${hit.comparisonMatches}/${hit.comparisonTotal} comparison`}><span>{hit.targetPosition}</span></button>)}</div>
        </div>
        {selectedHit&&<div className="hitDetail"><div><span>Tested ref position</span><b>{selectedHit.targetPosition??"Gap"}</b></div><div><span>{targetName} residue</span><b className="aa target">{selectedHit.targetAA}</b></div><div><span>Tested evidence</span><b>{selectedHit.targetPresent}/{selectedHit.targetTotal} ({Math.round(selectedHit.targetPresent/selectedHit.targetTotal*100)}%)</b></div><div><span>Comparison matches</span><b>{selectedHit.comparisonMatches}/{selectedHit.comparisonTotal}</b></div><div><span>Comparison ref</span><b>{selectedHit.referencePosition??"Gap"} · {selectedHit.referenceAA}</b></div><div><span>Other residue(s)</span><b>{selectedHit.comparisonAAs||"—"}</b></div></div>}
      </section>:null}
      {hits.length?<><div className="tableTools"><label>Filter results<input placeholder="Position or amino acid" value={query} onChange={e=>setQuery(e.target.value)}/></label><span>{visibleHits.length} shown</span></div><div className="tableWrap"><table><thead><tr><th>Alignment</th><th>Tested ref pos.</th><th>Tested ref AA</th><th>Comparison ref pos.</th><th>Comparison ref AA</th><th>{targetName} AA</th><th>Other AA(s)</th><th>Evidence</th></tr></thead><tbody>{visibleHits.map(h=><tr key={h.alignmentPosition} onClick={()=>setSelectedHit(h)} className={selectedHit?.alignmentPosition===h.alignmentPosition?"selectedRow":""}><td>{h.alignmentPosition}</td><td>{h.targetPosition??"Gap"}</td><td><span className="aa neutral">{h.targetReferenceAA}</span></td><td>{h.referencePosition??"Gap"}</td><td><span className="aa neutral">{h.referenceAA}</span></td><td><span className="aa target">{h.targetAA}</span></td><td><span className="aa comparison">{h.comparisonAAs}</span></td><td>{h.targetPresent}/{h.targetTotal} ({Math.round(h.targetPresent/h.targetTotal*100)}%); {h.comparisonMatches}/{h.comparisonTotal} comparison</td></tr>)}</tbody></table></div></>:<div className="empty"><b>No residues passed the selected rule.</b><p>Try a lower prevalence threshold, allow one or two comparison matches, or inspect the unmatched identifiers.</p></div>}
    </section>}
